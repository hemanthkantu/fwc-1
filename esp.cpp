...
void setup() {
  ...
  server.on("/triangle", HTTP_GET, [](AsyncWebServerRequest *request){
    request->send_P(200, "text/html", index_html);
  });

  server.on("/triangle", HTTP_POST, [&gpioval](AsyncWebServerRequest *request){
      
    int m = 2, n = 3 ,c4,c5,c6;
    double **A, **B, **C;
    ...
    // Creating the response string with line constants
    String response = "Medians:<br>c4: " + String(c4, 2) + "<br>c5: " + String(c5, 2) + "<br>c6: " + String(c6, 2) + "<br>";
